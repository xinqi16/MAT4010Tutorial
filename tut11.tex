...
\begin{example}[Failure of Banach-Steinhaus in Incomplete Spaces]
    Let $P$ denote the space of all real polynomials. Given a polynomial
    \[
        x(t) = \sum_{k=0}^{m}\alpha_{k}t^{k},
    \]
    we define the norm on $P$ as:
    \[
        \|x\| = \max_{0\le k\le m}|\alpha_{k}|.
    \]
    Define a sequence of linear functionals $T_n : P \to \mathbb{R}$ by truncating the series:
    \[
        T_{n}x = \sum_{k=0}^{\min\{m,n\}}\alpha_{k}.
    \]
    Then, the sequence is pointwise bounded, meaning
    \[
        \sup_{n\ge 0}|T_{n}x| < \infty \quad \forall x \in P.
    \]
    However, the sequence of operator norms is unbounded, i.e.,
    \[
        \sup_{n\ge 0}\|T_{n}\| = \infty.
    \]
    \textit{Remark:} This explicitly shows that the Banach-Steinhaus theorem fails when the space $X$ is not complete.
\end{example}
...
\begin{application}[Divergence of Lagrange Interpolation]
    For each integer $n\ge 0$, let $0\le t_{0} < t_{1} < \dots < t_{n}\le 1$ be any set of $(n+1)$ distinct nodes. The Lagrange interpolating operator $L_{n}:C[0,1]\rightarrow P[0,1]$ is defined by:
    \[
        L_{n}x(t_{k})=x(t_{k}), \quad 0\le k\le n, \quad \text{and} \quad \deg L_{n}x \le n.
    \]
    This interpolating polynomial can be explicitly written in the Lagrange basis form:
    \[
        L_{n}x(t) = \sum_{k=0}^{n}x(t_{k})p_{k}(t),
    \]
    where the basis polynomials are given by:
    \[
        p_{k}(t) = \prod_{j\ne k}^{n}\frac{t-t_{j}}{t_{k}-t_{j}}.
    \]
    We will establish two main results:
    \begin{enumerate}
        \item The operator norm (the Lebesgue constant) is exactly $\|L_{n}\| = \sup_{0\le t\le 1}\sum_{k=0}^{n}|p_{k}(t)|$.
        \item Given the known analytical result that $\|L_{n}\| \ge C \log n$ for some constant $C>0$, there exists a continuous function $x\in C[0,1]$ such that $\sup_{n\ge 0}\|L_{n}x\| = \infty$.
    \end{enumerate}
\end{application}
...
\begin{proof}
    ...
    \[
        \sum_{k=0}^{n} |p_k(\tau)| = \sup_{0\le t\le 1} \sum_{k=0}^{n} |p_k(t)|.
    \]
    ...
    \[
        x_0(t_k) = \operatorname{sgn} p_k(\tau) \quad \text{for} \quad 0 \le k \le n,
    \]
    ...
    \[
        \|L_{n}\| = \sup_{0\le t\le 1}\sum_{k=0}^{n}|p_{k}(t)|.
    \]
    ...
    \[
        \sup_{0\le t\le 1}\sum_{k=0}^{n}|p_{k}^{n}(t)| \ge C \log n.
    \]
    ...
    Consequently, there must exist at least one ``resonant" continuous function $x \in C[0,1]$ such that the sequence of its interpolations is unbounded:
    \[
        \sup_{n\ge 0} \|L_n x\| = \infty \quad \forall n\in\mathbb{N}.
    \]
    \textit{Remark:} This rigorously demonstrates that Lagrange interpolating polynomials do not universally converge uniformly to the original function, completely shattering the intuition that ``more nodes always yield better approximations".
\end{proof}
